...
script_path = os.path.join(current_script_dir, 'extract_soft_clip_seq.py')
    ...
    for iter in ['1', '2']:
        ...
        for i in ['1', '2']:
            ...
            if not os.path.exists(f'{outdir}/break_at_soft_clip/{iter}/{i}_hifi_asm_to_ref.sorted.bam'):
                subprocess.run(f'python {script_path} {bam} > {outdir}/break_at_soft_clip/{iter}/{i}_hifi_asm.fasta', shell=True)
...
def pipeline(ccs, outdir, threads):
    ...
    remove_softclips(outdir)

def run_pipeline(files, outdir):
    with open(files, "r") as file:
        samples = [line.strip().split()[0] for line in file]

    # Run function in parallel
    with Pool() as pool:
        pool.map(partial(run_pipeline, outdir=outdir), samples)
...
if __name__ == '__main__':       
    ...
    #honda_script_dir = os.path.dirname(os.path.abspath(__file__))
